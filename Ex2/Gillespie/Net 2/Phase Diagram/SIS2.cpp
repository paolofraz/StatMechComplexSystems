...
int count_SI_edges(int state[1000],int node1[1796], int node2[1796])
{	
	int count=0;
	for (int i=0;i<1796;++i)
	{
...
void infecting(int state[1000],int node1[1796],int node2[1796],int l)
{
	random_device rd;
	mt19937 gen((rd()));
...
int node1[1796];						//trovo i miei file 
int node2[1796];

ifstream iFile("net2_2.txt"); 
...
for(double lambda=0.;lambda<1.001;lambda=lambda+0.02)
{

		
//vector <double> inf_sim;					//inf medio per simulazione

for (int sim=0; sim<50; ++sim)	//numero di simulazioni da mediare sul parametro
{											
										//inizializza stato
	...
	{	
		...
		double sis=(1000-inf);
		double a1=0.003595595596*inf*sis*lambda;
		double a2=inf*mu;
		double a0=a1+a2;
		//cout<<"I:	"<<inf<<"\t"<<endl;
		//cout<<"S:	"<<sus<<endl;
